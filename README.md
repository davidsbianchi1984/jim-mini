...
| `POST /specialists/seed/tandem` | The **tandem hookup**: wire starter specialists to their QRME Starter Collection counterparts (`financial_stress` → `@marcus_bell`, `physical_distress` → `@dr_amara_osei`), resolving each @handle live against the connected QRME deployment — ids differ per deployment, handles are the stable cross-product names. Existing tandem links are kept; unresolved handles stay local; 409 without `JIM_QRME_URL`. `python -m jim.seed` runs it automatically when `JIM_QRME_URL` is set |
