...
const TABLE: Record<string, Partial<Record<Lang, string>>> = {
  // Asking a specialist through the tandem. The same rows stand verbatim
  // in the sibling consoles — one wording, one translation.
  //
  // This comment used to say "the footsteps chip in the corner", which was
  // wrong twice over: the chip left with the edge dock, and it never
  // labelled these keys in the first place.
  ...
  "sfy.auto.tick": {
    en: "JIM checks on its own every {n} seconds, whether or not this screen is open.", es: "JIM comprueba por su cuenta cada {n} segundos, esté o no abierta esta pantalla.", fr: "JIM vérifie de lui-même toutes les {n} secondes, que cet écran soit ouvert ou non.", de: "JIM prüft von selbst alle {n} Sekunden, ob dieser Bildschirm offen ist oder nicht.", pt: "O JIM verifica por conta própria a cada {n} segundos, esteja ou não este ecrã aberto.", it: "JIM controlla da solo ogni {n} secondi, che questa schermata sia aperta o no.", ja: "この画面が開いているかどうかに関わらず、JIMは{n}秒ごとに自ら確認します。", zh: "无论此屏幕是否打开，JIM 每 {n} 秒都会自行检查。", hi: "यह स्क्रीन खुली हो या नहीं, JIM हर {n} सेकंड में खुद जाँच करता है।", ar: "يتحقق JIM من تلقاء نفسه كل {n} ثانية، سواء كانت هذه الشاشة مفتوحة أم لا.",
  },
  "sfy.auto.notick": {
    en: "JIM's clock advances only while a screen is reading it. Turn the server's ticker on so it checks by itself.", es: "El reloj de JIM solo avanza mientras una pantalla lo está leyendo. Activa el temporizador del servidor para que compruebe por sí mismo.", fr: "L'horloge de JIM n'avance que lorsqu'un écran la lit. Activez le minuteur du serveur pour qu'il vérifie de lui-même.", de: "JIMs Uhr läuft nur, solange ein Bildschirm sie liest. Schalte den Taktgeber des Servers ein, damit es von selbst prüft.", pt: "O relógio do JIM só avança enquanto um ecrã o estiver a ler. Liga o temporizador do servidor para que verifique por si próprio.", it: "L'orologio di JIM avanza solo mentre una schermata lo legge. Attiva il temporizzatore del server perché controlli da solo.", ja: "JIMの時計は画面が読み取っている間だけ進みます。自ら確認させるにはサーバーのティッカーを有効にしてください。", zh: "JIM 的时钟只在有屏幕读取时才前进。请开启服务器的定时器，让它自行检查。", hi: "JIM की घड़ी तभी आगे बढ़ती है जब कोई स्क्रीन उसे पढ़ रही हो। सर्वर का टिकर चालू करें ताकि यह खुद जाँच करे।", ar: "لا تتقدم ساعة JIM إلا بينما تقرأها شاشة. شغّل مؤقّت الخادم ليتحقق من تلقاء نفسه.",
  },
  "sfy.auto.off": {
