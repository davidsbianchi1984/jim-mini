...
def row(x, y, w, ic, col, k, s):
    ...
    return (rrect(x, y, w, 40, 12, "url(#gCard)", C["line"], 1)
            + rrect(x + 8, y + 9, 22, 22, 7, c + "28") + icon(ic, x + 19, y + 20, c, 0.6)
...
def render(s):
    ...
    if h == "home":
        ...
        for i, (r, col, pct) in enumerate([(40, C["green"], .75), (30, C["brandA"], .6), (20, C["amber"], .55)]):
            circ = 2 * 3.14159 * r
            o.append(f'<circle cx="{cx}" cy="{y+48}" r="{r}" fill="none" stroke="{col}22" stroke-width="9"/>')
