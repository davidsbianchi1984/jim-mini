...
<tr>
<td valign="top">9</td>
<td valign="top">A coach that answers on the device is private only until it meets a question it cannot answer. At that point the usual product hands the person's question, as written and with the context around it, to a vendor's model — and the only record that anything went is the vendor's.</td>
<td valign="top">The <strong>general-terms door</strong> (<code>egress.py</code>). Under its own permit, <code>ask_in_general_terms</code>, a coach turn is answered from the offline store first; only a miss reaches a model, and what reaches it is the question <strong>recomposed</strong>: the person's names, contacts, links, phones, dates, times, ages, readings and every bare number taken out in a fixed order of passes, each replaced by a marker that says what it was, and the first person turned into the third. What was taken out is kept beside the row and never sent. The answer is learned into the store (<code>excursions.learned</code>) and the recorded miss is closed, so the same gap is asked once. A degrade to the local stub is not learned. Offline mode is not opened by the permit.</td>
<td valign="top">A <strong>ledger a person reads</strong>: one row in <code>egress</code> per sentence that could leave — the sentence exactly as sent, the fixed framing sent with it, what was taken out, the provider it went to, who answered and whether it left the host. Written by the two doors anything goes out by, the study path and the model door, so a turn sent as written under no permit is on it too; served by <code>GET /egress/{user}</code> and on the coach screen. Every other place a provider is spoken to is named in the test, so a third door has to be added on purpose.</td>
<td valign="top"><code>jim/<wbr>egress.py</code>,<br><code>jim/<wbr>coach.py</code>,<br><code>jim/<wbr>research.py</code>,<br><code>jim/<wbr>llm.py</code> — <code>test_<wbr>the_<wbr>coach_<wbr>asks_<wbr>in_<wbr>general_<wbr>terms.py</code></td>
</tr>
</tbody>
</table>
...
| The coach asks in general terms, and every sentence that left is on the record | A coach that is private until it meets a question it cannot answer hands that question, as written, to a vendor. | `jim/egress.py` recomposes a miss into a sentence about *a person* — names, contacts, links, phones, dates, times, ages, readings and bare numbers out, first person to third — before anything leaves, learns the answer so the gap is asked once, and writes one `egress` row per sentence that could leave, word for word, read at `GET /egress/{user}`. | `test_the_coach_asks_in_general_terms.py` | 07 |
